...
export default function UploadPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        courseCode: '',
        content: '',
        quizJson: '' // For the optional quiz
    });
    ...
    useEffect(() => {
        // Filter courses based on university and search
        // Flatten the catalog for the selected Uni
        if (!university) return;

        let allCourses = [];
        const uniCatalog = COURSE_CATALOG[university] || {};
        Object.values(uniCatalog).forEach(programs => { // BSCS, BSSE...
            Object.values(programs).forEach(semCourses => {
                allCourses = [...allCourses, ...semCourses];
            });
        });

        // Unique by code
        allCourses = Array.from(new Map(allCourses.map(item => [item.code, item])).values());
