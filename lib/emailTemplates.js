...
export const getStreakReminderTemplate = (studentName, streakCount) => {
    return `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 2px solid #ff9600; border-radius: 12px; background-color: #fffaf0;">
            <h2 style="color: #ff9600; text-align: center; font-size: 1.8rem;">🔥 Streak At Risk!</h2>
            <p style="text-align: center; font-size: 1.2rem;">Hi <strong>${studentName}</strong>, don't let your ${streakCount}-day streak zero out!</p>
            
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #ff9600; text-align: center;">
                <div style="font-size: 3rem; margin-bottom: 10px;">📉</div>
                <p style="margin: 0; color: #333; font-size: 1.1rem;">You haven't completed a lesson yet today.</p>
                <p style="margin: 10px 0 0 0; color: #777;">Streak freezes are not available. If you miss today, you start from 0.</p>
            </div>

            <div style="text-align: center; margin-top: 30px;">
                <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/learning" style="background: #ff9600; color: white; text-decoration: none; padding: 14px 28px; border-radius: 50px; font-weight: bold; font-size: 1.2rem; box-shadow: 0 4px 0 #cc7a00; display: inline-block;">
                    WATCH A LESSON NOW
                </a>
            </div>

            <p style="margin-top: 30px; font-size: 0.8rem; color: #aaa; text-align: center;">
                Go to "Me" > Settings to unsubscribe from reminders.
            </p>
        </div>
    `;
};
